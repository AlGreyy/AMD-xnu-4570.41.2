...
//    info_p->cpuid_logical_per_package = cores;//cores;
    ...
    info_p->core_count = info_p->cpuid_cores_per_package; //bitfield32(reg[ebx], 7, 0);
...
cpuid_set_generic_info(i386_cpu_info_t *info_p)
{
    ...
    DBG(" cpuid_processor_flag           : %d\n", info_p->cpuid_processor_flag);
    ...
    if (info_p->cpuid_max_ext >= 0x80000001) {
        ...
        if (IsIntelCPU())
        {
            ...
            info_p->cpuid_extfeatures = quad(reg[ecx], reg[edx]) & ~CPUID_EXTFEATURE_XD;
        }
    }
    ...
    if (info_p->cpuid_model >= CPUID_MODEL_IVYBRIDGE ) { //|| (IsAmdCPU() && info_p->cpuid_family >= 21)
        /*
         * Leaf7 Features:
         */
        cpuid_fn(0x7, reg);
