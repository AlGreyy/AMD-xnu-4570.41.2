...
cpuid_set_AMDcache_info( i386_cpu_info_t * info_p )
{
    ...
    if (info_p->cpuid_family >= 0x8)
    {
        //uint32_t cores = 0;
        uint32_t logical = 0;
        
        cpuid_fn(0x8000001E, regt);
        //cores = bitfield32(regt[ebx], 7, 0);
        logical = bitfield32(regt[ebx], 15, 8) + 1;
        cpuid_fn(0x80000008, reg);
        info_p->cpuid_logical_per_package = (bitfield32(reg[ecx], 7, 0) + 1) / logical;
    }
    else
    {
